...
Lote D1 de `docs/PLANO_VOLUMETRIA_DW_DIRETO.md`. Porte de
`nuvem-ia catering/carga/fonte_oracle.py`, que já roda em produção desde
25/ago/2026: o que está aqui foi provado lá, não inventado agora.

## O que este arquivo NÃO é

Ele não substitui o `conexao.py` neste lote. Enquanto o D3 não reescrever o SQL,
os dois existem: `conexao.py` fala com o `nuvem-db` (Postgres) e serve a tela,
este fala com o DW e serve só o diagnóstico. A troca é o D3, e a razão de não
antecipá-la é a comparação Postgres × Oracle que prova a tradução do SQL — ela
precisa dos dois de pé.
...
3. **dois testes**, em `tests/test_volumetria_catering_dw.py`: um **estático**,
...
def credencial() -> tuple[str, str]:
    ...
    if faltando:
        raise CredencialAusente(
            f"volumetria de catering não configurada neste ambiente: falta "
...
def conectar():
    ...
    try:
        ...
        raise DWIndisponivel(
            ...
            "catering lê o DW direto; o resto do Hub continua funcionando. Se "
