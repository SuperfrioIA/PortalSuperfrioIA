...
Cópia de `backend/volumetria_transporte/schema_dw.py`, mesma adaptação da
irmã para **uma tabela só** — sem `movimento` para iterar. A docstring longa
sobre POR QUE cada conferência existe está em
`backend/volumetria_catering/schema_dw.py` (D1); aqui só o que muda.
...
class ContratoDivergenteDW(Exception):
    ...
    def __init__(self, problemas):
        ...
        super().__init__(
            "contrato da volumetria de estoque divergente do DW — "
            f"{len(self.problemas)} problema(s): " + "; ".join(self.problemas)
            + ". Atualize backend/volumetria_estoque/contrato.py (e confirme "
...
def verificar(cur) -> list[str]:
    ...
    for aviso in visto["avisos"]:
        logger.warning("volumetria-estoque/DW: %s", aviso)
    return visto["avisos"]
