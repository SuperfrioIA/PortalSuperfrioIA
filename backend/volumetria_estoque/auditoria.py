...
Cópia de `backend/volumetria_transporte/auditoria.py`, outra tabela
(`volumetria_estoque_downloads`, `models.py`)."""
...
from backend.volumetria_estoque.models import FORMATOS, VolumetriaEstoqueDownload
...
def abrir(recorte, formato, ip=None, usuario=None) -> int:
    ...
    with db() as session:
        registro = VolumetriaEstoqueDownload(
            usuario=usuario or "-",
            formato=formato,
...
def falhar(registro: int, erro) -> None:
    ...
    logger.warning("auditoria volumetria-estoque %s: download falhou -- %s", registro, erro)


def _atualizar(registro: int, status: str, linhas=None, erro=None) -> None:
    with db() as session:
        linha = session.get(VolumetriaEstoqueDownload, registro)
        if linha is None:
            logger.warning("auditoria volumetria-estoque %s: registro não encontrado", registro)
            return
        linha.status = status
...
def _como_dict(linha: VolumetriaEstoqueDownload) -> dict:
    return {
        "id": linha.id,
...
def listar(limite: int = 100) -> list[dict]:
    with db() as session:
        linhas = session.execute(
            select(VolumetriaEstoqueDownload)
            .order_by(VolumetriaEstoqueDownload.id.desc())
            .limit(limite)
        ).scalars().all()
