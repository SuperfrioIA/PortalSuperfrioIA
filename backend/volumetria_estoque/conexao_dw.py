...
exatamente por isso que ele é a única coisa que três módulos (catering,
transporte, estoque) ganharam duplicada sem discussão: extrair pra
`backend/dw_consulta/` fica para o T1, se algum dia acontecer.

**Diferente do catering, este módulo não tem `conexao.py` (Postgres)** — não
existe fonte intermediária para estoque, então esta é a ÚNICA conexão.
...
3. **dois testes**, em `tests/test_volumetria_estoque_dw.py`: um **estático**,
...
def credencial() -> tuple[str, str]:
    ...
    if faltando:
        raise CredencialAusente(
            f"volumetria de estoque não configurada neste ambiente: falta "
...
def conectar():
    ...
    try:
        ...
        raise DWIndisponivel(
            ...
            "estoque lê o DW direto; o resto do Hub continua funcionando. Se "
